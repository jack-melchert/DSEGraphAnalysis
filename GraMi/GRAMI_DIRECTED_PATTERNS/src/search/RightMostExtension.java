	...
	protected final void extend(final DFSCode<NodeType, EdgeType> code) 
	{
		...
		// find extensions of the last node;
		{
			...
			for (int i = 0; i < pairs.size(); i++) 
			{
				...
				final GSpanEdge<NodeType, EdgeType> gEdge = new GSpanEdge<NodeType, EdgeType>().set(lastNode, lastNode+1 , sortedFreqLabels.indexOf(theLabelA), 0, index, 1, theLabelA, label); 											
			...
			ArrayList<Integer> connected= lastVariable.getDistanceConstrainedWith();
			...
			for (int i = 0; i < vrs.length; i++) 
			{
				...
				if(labelDC.contains(candidateVB.getLabel()))
				{
					...
					{
						Variable connectVB=vrs[connected.get(j)];
					...
					final GSpanEdge<NodeType, EdgeType> gEdge = new GSpanEdge<NodeType, EdgeType>().set(lastNode, candidateVB.getID() , sortedFreqLabels.indexOf(theLabelA), 0, sortedFreqLabels.indexOf(theLabelB), 1, theLabelA, theLabelB); 											
					if ((code.getLast().compareTo(gEdge) < 0))
					{
						add(gEdge, code,1);
					}
		...
		do {
			...
			for (int i = 0; i < pairs.size(); i++) 
			{
				...
				int label=currentPoint.x;
				int index=currentPoint.y;
				final GSpanEdge<NodeType, EdgeType> gEdge = new GSpanEdge<NodeType, EdgeType>().set(ackNode, lastNode+1 , sortedFreqLabels.indexOf(theLabelA), 0, index, 1, theLabelA, label);
				add(gEdge, code, 0);
			}
		} while (ackNode > 0);
		/////////////////////////////////The other ONe
		...
		do {
			...
			Variable currentVariable= vrs[ackNode];
			//System.out.println(ackNode+" has to be = "+currentVariable.getID());  //assertion						
